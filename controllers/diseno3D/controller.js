...
const creardiseno3D = async (datosdiseno3D, callback) => {
    if (
      ...
      await baseDeDatos.collection("diseno3D").insertOne(datosdiseno3D, callback);
    } else {
      return "error"; 
    }
};
...
const editardiseno3D = async (id, edicion, callback) => {
  ...
  await baseDeDatos.collection("diseno3D").findOneAndUpdate(filtrodiseno3D,operacion,
